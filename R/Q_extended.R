...
generate_Q_with_interactions <- function(Q,
  ...
  if (verbose) {
    message("=== Q-MATRIX INTERACTION GENERATION ===")
    message("Original Q-matrix dimensions: ", I, " items x ", K, " rules")
    message("Number of components: ", length(M_list))
  }
  ...
  for (m in seq_along(M_list)) {
    ...
    if (length(rules_comp) < 2) {
      if (verbose) {
        message("Component ", m, ": Only ", length(rules_comp),
    ...
    for (i in 1:(length(rules_comp) - 1)) {
      for (j in (i + 1):length(rules_comp)) {
        rule_idx_i <- rules_comp[i]
        rule_idx_j <- rules_comp[j]

        # Create interaction term
        new_interaction <- Q[, rule_idx_i] * Q[, rule_idx_j]
        non_zero_count <- sum(new_interaction)

        # Only add interaction if it has non-zero entries
        if (non_zero_count > 0) {
          Q_extended <- cbind(Q_extended, new_interaction)
          rule_counter <- rule_counter + 1

          interaction_label <- paste0("Inter_", rule_idx_i, "*", rule_idx_j)
          new_rule_names <- c(new_rule_names, interaction_label)
          ...
          interaction_log <- rbind(interaction_log, data.frame(
            ...
            rule_i = rule_idx_i,
            rule_j = rule_idx_j,
            interaction_name = interaction_label,
    ...
    if (verbose) {
      message("Component ", m, ": Generated ", component_interactions, " interactions")
    }
  }
  ...
  if (verbose) {
    message("Total interactions generated: ", interactions_generated)
    message("Extended Q-matrix dimensions: ", I, " items x ", ncol(Q_extended), " rules")
  }
  ...
  perform_collinearity_diagnostics <- function(X, label = "") {
    if (verbose && label != "") {
      message("=== COLLINEARITY DIAGNOSTICS: ", label, " ===")
    }
    ...
    if (verbose) {
      message("Matrix rank: ", rank_X, "/", ncol(X), " ",
    ...
    if (verbose) {
      message("Condition indices range: [", round(min(cond_indices), 2), ", ",
          round(max(cond_indices), 2), "]")
      message("Eigenvalues below ", min_eigenvalue, ": ", low_eigenvalues)
      if (!is.null(vif_values)) {
        message("VIF values above 10: ", high_vif)
      }
    }

    # Warnings (always shown regardless of verbose)
    if (!full_rank) {
      warning("Matrix is rank deficient!")
    }
    if (severe_collinearity) {
      warning("Severe collinearity detected (CI > ", max_condition_index, ")!")
    } else if (moderate_collinearity) {
      warning("Moderate collinearity detected (CI > 10)!")
    }
    if (low_eigenvalues > 0) {
      warning(low_eigenvalues, " eigenvalue(s) below ", min_eigenvalue)
    }
  ...
  if (diagnostics_original$severe_collinearity || !diagnostics_original$full_rank) {
    if (verbose) {
      message("=== ATTEMPTING TO RESOLVE COLLINEARITY ===")
    }
    ...
    if (length(interaction_indices) > 0) {
      ...
      repeat {
        ...
        if (verbose) {
          message("Iteration ", iteration, ": Removing ", colnames(Q_extended)[worst_col])
        }
      }
      ...
      if (length(to_remove) > 0 && length(to_remove) < length(interaction_indices)) {
        ...
        if (verbose) {
          message("Successfully removed ", length(removed_interactions), " interactions:")
          message(paste(removed_interactions, collapse = ", "))
        }
      } else {
        warning("Could not resolve collinearity issues without removing all interactions.")
        if (save_to_global) {
  ...
  if (save_to_global) {
    ...
    if (verbose) {
      message("Results saved to .GMLTM_env (accessible via the returned list):")
      message("- Q_extended: Final extended Q-matrix (", nrow(Q_improved), "x", ncol(Q_improved), ")")
      message("- M_list: Updated component list")
      message("- Q_extended_original: Original extended matrix")
      message("- M_list_original: Original component list")
      if (!is.null(.GMLTM_env$Q_extended_failed)) {
        message("- Q_extended_failed: Failed correction attempt")
      }
    }
  }
  ...
  if (plot_diagnostics) {
    ...
    tryCatch({
      ...
      if (verbose) message("Generating condition indices plot...")
      ...
      if (verbose) message("Generating eigenvalues plot...")
      ...
      if (verbose) message("Generating correlation heatmap...")
      ...
      if (!is.null(diagnostics_improved$vif_values)) {
        ...
        if (length(vif_vals) > 0) {
          if (verbose) message("Generating VIF plot...")
      ...
      if (verbose) {
        message("Diagnostic plots generated successfully!")
        message("- Use result$plots$condition_indices() to recreate condition indices plot")
        message("- Use result$plots$eigenvalues() to recreate eigenvalues plot")
        message("- Use result$plots$correlation_heatmap() to recreate correlation heatmap")
        if (!is.null(plots$vif_values)) {
          message("- Use result$plots$vif_values() to recreate VIF plot")
        }
      }

    }, error = function(e) {
      warning("Could not generate some diagnostic plots: ", e$message)
  ...
  if (verbose) {
    message("=== SUMMARY ===")
    message("Original Q-matrix: ", result$summary$original_dimensions[1], " x ",
        result$summary$original_dimensions[2])
    message("Extended Q-matrix: ", result$summary$extended_dimensions[1], " x ",
        result$summary$extended_dimensions[2])
    message("Interactions generated: ", result$summary$interactions_generated)
    message("Interactions removed: ", result$summary$interactions_removed)
    message("Final collinearity status: ", result$summary$final_collinearity_status)
    message("=======================================")
  }
